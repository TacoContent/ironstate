...
<#
.SYNOPSIS
    Downloads and installs the latest (or a specific) ironstate release
...
function Test-ScriptChecksum {
	...
	$checksumUrl = "https://raw.githubusercontent.com/$Repo/main/install/install.ps1.sha256"
	try {
		$expected = ((Invoke-RestMethod -Uri $checksumUrl -Headers @{ "User-Agent" = "repo-install-script" }).Trim() -split '\s+')[0].ToLowerInvariant()
	} catch {
...
function Exit-Unsupported {
	...
	Write-Host ""
	Write-Host "No supported release was found for this machine."
...
function Invoke-Install {
	...
	if ($Version) {
		...
		try {
			$release = Invoke-RestMethod -Uri "https://api.github.com/repos/$Repo/releases/latest" -Headers @{ "User-Agent" = "repo-install-script" }
	...
	$tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) ("$Binary-install-" + [System.Guid]::NewGuid().ToString("N"))
