...
function Fail-Unsupported {
	...
	Write-Host ""
	Write-Host "No supported ironstate release was found for this machine."
...
if ($Version) {
	...
	try {
		$release = Invoke-RestMethod -Uri "https://api.github.com/repos/$Repo/releases/latest" -Headers @{ "User-Agent" = "ironstate-install-script" }
...
$tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) ("ironstate-install-" + [System.Guid]::NewGuid().ToString("N"))
